...
var mapPctFull = function(data, stations, key_used) {
    var sorted = d3.nest()
        .key(function(d) {
...
fs.readFile('station_data/all_resv.csv', 'utf8', function(e, stations) {
    var stat = d3.csv.parse(stations);

    fs.readFile('data/all.csv', 'utf8', function(e, water) {
        var data = d3.csv.parse(water);

        var enhanced_stations = mapPctFull(data, stat, true);

        var options = {header: true,
            columns: ['reservoir','lat','lng','state', 'pct_capacity', 'capacity']
        };

        stringify(enhanced_stations, options, function(e, output){
            fs.writeFile('station_data/enhanced_stations.csv', output, function(err) {
                console.log(err)
            });
        });
